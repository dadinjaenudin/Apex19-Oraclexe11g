...
ADD apex.zip /tmp/apex.zip
WORKDIR /tmp
RUN unzip /tmp/apex.zip
ADD apxchpwd_custom.sql /tmp/apxchpwd_custom.sql
ADD apex.sh /tmp/apex.sh
RUN chmod +x /tmp/apex.sh

VOLUME ["$ORACLE_BASE/oradata"]
EXPOSE 1521 8080 8181
